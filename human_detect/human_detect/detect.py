    ...
    def image_callback(self, msg):
        ...
        for idx, img in enumerate(images):
            ...
            for result in results[0].boxes:
                ...
                confidence = float(result.conf.cpu().numpy())  # 将confidence转换为浮点数
                person_center = self.get_person_center(box)
                self.draw_box_with_label(img, box, f'Person {confidence:.2f}', color=(255, 0, 0))  # 蓝色框 + 置信度

                # # 保存带有检测框和标签的图片
                # self.save_detected_image(img, frame_id, idx + 1)

                # 检测人脸
                person_img = img[int(box[1]):int(box[3]), int(box[0]):int(box[2])]
                ...
                for face in faces:
                    face_box = [
                        face[0] + box[0], face[1] + box[1],
                        face[2] + box[0], face[3] + box[1]
                    ]
                    # 计算人脸角度（0-360度）
                    face_center = self.get_person_center(face_box)
                    face_angle = float(self.calculate_face_angle(face_center, idx))
                    # 矫正度数处理
                    face_angle = float(face_angle - 45) % 360

                    # 判断人脸是否面向相机
                    if self.is_facing_camera(face_center, person_center):
                        # 计算到相机的距离
                        # distance = person_center[1]  # 使用y坐标作为距离的简单估计
                        face_area = (face_box[2] - face_box[0]) * (face_box[3] - face_box[1])  # 计算人脸的面积
                        distance = 1.0 / face_area  # 使用面积的倒数来近似表示距离

                        if distance < closest_distance:
                            closest_distance = distance
                            closest_face_angle = face_angle

                    # 绘制人脸框以及角度
                    self.draw_box_with_label(img, face_box, f'Face {face_angle:.2f} deg', color=(0, 255, 0))  # 绿色框 + 角度
